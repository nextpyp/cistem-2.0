...
void CtffindApp::DoInteractiveUserInput( ) {
    ...
    bool        known_tilt                         = false;
    float       known_tilt_angle                   = 0.0;
    float       known_tilt_axis_angle              = 0.0;
    ...
    if ( old_school_input || old_school_input_ctffind4 ) {
        ...
        if ( ! find_additional_phase_shift ) {
            ...
            if (determine_tilt) {
                known_tilt = my_input->GetYesNoFromUser("Already determine sample tilt?", "Answer yes if you already determined the tilt angle and axis angle.", "No");
                if ( known_tilt ) {
                    known_tilt_angle = my_input->GetFloatFromUser("Pre-determined tilt angle", "Enter the angle if you already determine the tilt angle (i.e. tilt-series alignment)", "0.0", -90.0, 90.0);
                    known_tilt_axis_angle = my_input->GetFloatFromUser("Pre-determined tilt axis angle", "Enter the angle if you already determine the tilt axis angle (i.e. tilt-series alignment)", "0.0", -360.0, 360.0);
                }
            }
        }
    ...
    my_current_job.ManualSetArguments("tbitffffifffffbfbfffbffbbsbsbfffbfffbbffiii", input_filename.c_str( ), //1
                                      input_is_a_movie,
...
bool CtffindApp::DoCalculation( ) {
    ...
    bool              determine_tilt                     = my_current_job.arguments[36].ReturnBoolArgument( );
    const bool        known_tilt                         = my_current_job.arguments[37].ReturnBoolArgument( );
    const float       known_tilt_angle                   = my_current_job.arguments[38].ReturnFloatArgument( );
    float             known_tilt_axis_angle              = my_current_job.arguments[39].ReturnFloatArgument( );
    int               desired_number_of_threads          = my_current_job.arguments[40].ReturnIntegerArgument( );
    int               eer_frames_per_image               = my_current_job.arguments[41].ReturnIntegerArgument( );
    int               eer_super_res_factor               = my_current_job.arguments[42].ReturnIntegerArgument( );
    

    // restrict tilt axis angle to range from -180 to +180
    known_tilt_axis_angle = (known_tilt_axis_angle < -180) ? 360 + known_tilt_axis_angle : known_tilt_axis_angle;
    known_tilt_axis_angle = (known_tilt_axis_angle > 180) ? 360 - known_tilt_axis_angle : known_tilt_axis_angle;
    ...
    float best_fit_resolution = std::numeric_limits<double>::infinity();;
    int times = determine_tilt ? 2 : 1;
    
    for (int i = 0; i < times; i++) {
        if (i == 1) {determine_tilt = false;}
        ...
        if ( amplitude_spectrum_input || filtered_amplitude_spectrum_input ) {
            ...
            if ( determine_tilt ) {
                ...
                if (!known_tilt) { 
                    ...
                    tilt_scorer.best_tilt_axis = known_tilt_axis_angle;
                    tilt_scorer.best_tilt_angle = known_tilt_angle;
                }
                // tilt_scorer.SearchTiltAxisAndAngle( );
        ...
        // HF Liu - only ouput the best estimation (using tilt angles or not using tilt angles)
        float fit_to_resolution = pixel_size_for_fitting / spatial_frequency[last_bin_with_good_fit];


        if (fit_to_resolution < best_fit_resolution) {
        ...
        if ( is_running_locally ) {
            if (i == 1 && fit_to_resolution >= best_fit_resolution) {
                // instead write the previous one
            ...
            if (!determine_tilt && fit_to_resolution < best_fit_resolution) {output_text->WriteLine(values_to_write_out);}
            if ( (! old_school_input) && number_of_micrographs > 1 && is_running_locally )
        ...
        if (fit_to_resolution < best_fit_resolution) {
            best_fit_resolution = fit_to_resolution;
        }
