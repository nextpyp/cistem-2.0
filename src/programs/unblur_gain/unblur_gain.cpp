...
void UnBlurApp::DoInteractiveUserInput( ) {
    ...
    bool        gain_flipv                         = false;
    bool        gain_fliph                         = false;
    int         gain_rotation                      = 0;
    ...
    if ( set_expert_options == true ) {
        ...
        if ( ! movie_is_gain_corrected ) {
            ...
            gain_flipv    = my_input->GetYesNoFromUser("Gain to flip along Y", "Flip gain reference vertically", "no");
            gain_fliph    = my_input->GetYesNoFromUser("Gain to flip along X", "Flip gain reference horizontally", "no");
            gain_rotation = my_input->GetIntFromUser("Rotate gain clockwise by 90 degree", "Number of times to rotate gain reference 90 degree", "0", 0);
        }
        ...
        forceinteger = my_input->GetYesNoFromUser("Forcing shifts as integers?", "If yes, round the shifts", "no");
    }
    else {
    ...
    my_current_job.ManualSetArguments("ttfffbbfifbiifffbsbbibsfbfffbtbtiiiibttiib", input_filename.c_str( ),
...
bool UnBlurApp::DoCalculation( ) {
    ...
    bool        gain_flipv                           = my_current_job.arguments[18].ReturnBoolArgument( );
    bool        gain_fliph                           = my_current_job.arguments[19].ReturnBoolArgument( );
    int         gain_rotation                        = my_current_job.arguments[20].ReturnIntegerArgument( );
    bool        movie_is_dark_corrected              = my_current_job.arguments[21].ReturnBoolArgument( );
    wxString    dark_filename                        = my_current_job.arguments[22].ReturnStringArgument( );
    float       output_binning_factor                = my_current_job.arguments[23].ReturnFloatArgument( );
    bool        correct_mag_distortion               = my_current_job.arguments[24].ReturnBoolArgument( );
    float       mag_distortion_angle                 = my_current_job.arguments[25].ReturnFloatArgument( );
    float       mag_distortion_major_scale           = my_current_job.arguments[26].ReturnFloatArgument( );
    float       mag_distortion_minor_scale           = my_current_job.arguments[27].ReturnFloatArgument( );
    bool        write_out_amplitude_spectrum         = my_current_job.arguments[28].ReturnBoolArgument( );
    std::string amplitude_spectrum_filename          = my_current_job.arguments[29].ReturnStringArgument( );
    bool        write_out_small_sum_image            = my_current_job.arguments[30].ReturnBoolArgument( );
    std::string small_sum_image_filename             = my_current_job.arguments[31].ReturnStringArgument( );
    int         first_frame                          = my_current_job.arguments[32].ReturnIntegerArgument( );
    int         last_frame                           = my_current_job.arguments[33].ReturnIntegerArgument( );
    int         number_of_frames_for_running_average = my_current_job.arguments[34].ReturnIntegerArgument( );
    int         max_threads                          = my_current_job.arguments[35].ReturnIntegerArgument( );
    bool        saved_aligned_frames                 = my_current_job.arguments[36].ReturnBoolArgument( );
    std::string aligned_frames_filename              = my_current_job.arguments[37].ReturnStringArgument( );
    std::string output_shift_text_file               = my_current_job.arguments[38].ReturnStringArgument( );
    int         eer_frames_per_image                 = my_current_job.arguments[39].ReturnIntegerArgument( );
    int         eer_super_res_factor                 = my_current_job.arguments[40].ReturnIntegerArgument( );
    bool        forceinteger                         = my_current_job.arguments[41].ReturnBoolArgument( );
	...
	*/
    ...
    if ( ! movie_is_gain_corrected ) {
        ...
        // gain reference operation
        if (gain_flipv) 
            gain_image.ApplyMirrorAlongY( );
        else if (gain_fliph) 
            gain_image.InvertPixelOrder( );
        
        if (gain_rotation > 0) {
            // negative 90 degree as clockwise
            for (int i=1; i <= gain_rotation; i++) {
                gain_image.RotateInPlaceAboutZBy90Degrees(false, false);
                }
            }
    }
    if ( ! movie_is_dark_corrected ) {
...
#pragma omp for
        ...
        for ( image_counter = 0; image_counter < number_of_images; image_counter++ ) {
            ...
            if (forceinteger) {
                current_x_shifts[image_counter] = round(current_x_shifts[image_counter]);
                current_y_shifts[image_counter] = round(current_y_shifts[image_counter]);
            }
